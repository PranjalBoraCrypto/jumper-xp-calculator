...
${sparks}
  ...
  // Jumper Pass levels — same thresholds Jumper's profile page uses (level = f(XP)).
  const LEVELS = [[1,0,10],[2,10,20],[3,20,50],[4,50,75],[5,75,100],[6,100,150],[7,150,225],[8,225,350],[9,350,500],[10,500,700],[11,700,900]];
  for (let l = 12, lo = 900; l <= 100; l++, lo += 200) LEVELS.push([l, lo, lo + 200]);
  function levelFor(points) {
    if (!points) return { level: 0, min: 0, max: 10, next: 10, progress: 0 };
    const row = LEVELS.find(([, lo, hi]) => points >= lo && points <= hi) || LEVELS[LEVELS.length - 1];
    const [level, min, max] = row; return { level, min, max, next: Math.max(0, max - points), progress: Math.min(1, Math.max(0, (points - min) / (max - min))) };
  }
  ...
  return { TIERS, tierFor, emblemSVG, LOGO, LEVELS, levelFor };
})();
